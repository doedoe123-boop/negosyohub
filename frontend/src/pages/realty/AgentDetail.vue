  ...
  <div class="min-h-screen bg-slate-50">
    ...
    <div v-if="loading" class="animate-pulse">
      ...
      <div class="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <div class="relative -mt-16 flex items-end gap-5">
          <div class="size-32 shrink-0 rounded-full bg-slate-300 ring-4 ring-white shadow-md" />
          <div class="mb-2 space-y-2">
            <div class="h-8 w-64 rounded-lg bg-slate-200" />
            <div class="h-4 w-32 rounded-full bg-slate-100" />
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <div class="mx-auto mb-6 flex size-20 items-center justify-center rounded-full bg-slate-100">
        <ShieldCheckIcon class="size-10 text-slate-400" />
      </div>
    ...
    <template v-else>
      ...
      <div class="mx-auto max-w-6xl px-4 sm:px-6 pb-20">
        <!-- Main Info Overlap -->
        <div class="relative -mt-20 mb-8 rounded-3xl bg-white p-6 sm:p-8 shadow-sm ring-1 ring-slate-100">
          <div class="flex flex-col gap-6 sm:flex-row sm:items-start">
            
            <!-- Photo -->
            <div class="shrink-0 text-center">
              <div class="relative mx-auto size-32 sm:size-40 rounded-full bg-white ring-4 ring-white shadow-lg overflow-hidden">
                <img
              ...
              <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div>
                  ...
                  <div class="mt-3 flex flex-wrap items-center gap-3 text-sm text-slate-500">
                    ...
                    <span v-if="agent.prc_license_number" class="hidden sm:inline text-slate-300">•</span>
                    <span v-if="agent.prc_license_number" class="font-medium text-slate-600">
                ...
                <div class="shrink-0">
                  <a
                    ...
                    class="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-emerald-600 px-6 py-3 text-sm font-bold text-white shadow-sm transition-all hover:bg-emerald-700 hover:shadow-emerald-600/25 hover:shadow-md"
                  >
          ...
          <div class="grid gap-8 lg:grid-cols-[1fr_300px]">
            <div>
              <h3 class="font-bold text-slate-900 mb-2">About the Agent</h3>
            ...
            <div v-if="agent.social_links && Object.entries(agent.social_links).some(([_, v]) => v)">
              <h3 class="font-bold text-slate-900 mb-3">Connect Online</h3>
              <div class="flex flex-wrap gap-2">
                <a v-if="agent.social_links.facebook" :href="formatSocialUrl(agent.social_links.facebook)" target="_blank" class="flex size-10 items-center justify-center rounded-xl bg-slate-50 text-slate-500 hover:bg-blue-50 hover:text-blue-600 transition-colors" title="Facebook">
                ...
                <a v-if="agent.social_links.instagram" :href="formatSocialUrl(agent.social_links.instagram)" target="_blank" class="flex size-10 items-center justify-center rounded-xl bg-slate-50 text-slate-500 hover:bg-pink-50 hover:text-pink-600 transition-colors" title="Instagram">
                ...
                <a v-if="agent.social_links.linkedin" :href="formatSocialUrl(agent.social_links.linkedin)" target="_blank" class="flex size-10 items-center justify-center rounded-xl bg-slate-50 text-slate-500 hover:bg-sky-50 hover:text-sky-700 transition-colors" title="LinkedIn">
                ...
                <a v-if="agent.social_links.tiktok" :href="formatSocialUrl(agent.social_links.tiktok)" target="_blank" class="flex size-10 items-center justify-center rounded-xl bg-slate-50 text-slate-500 hover:bg-slate-900 hover:text-white transition-colors" title="TikTok">
                ...
                <a v-if="agent.social_links.youtube" :href="formatSocialUrl(agent.social_links.youtube)" target="_blank" class="flex size-10 items-center justify-center rounded-xl bg-slate-50 text-slate-500 hover:bg-red-50 hover:text-red-600 transition-colors" title="YouTube">
                ...
                <a v-if="agent.social_links.website" :href="formatSocialUrl(agent.social_links.website)" target="_blank" class="flex size-10 items-center justify-center rounded-xl bg-slate-50 text-slate-500 hover:bg-emerald-50 hover:text-emerald-600 transition-colors" title="Personal Website">
        ...
        <h2 class="mb-4 text-xl font-bold text-slate-900">Featured Properties</h2>
        
        <div v-if="properties.length === 0" class="rounded-3xl border border-dashed border-slate-300 py-16 text-center text-slate-400">
          <HomeModernIcon class="mx-auto size-12 mb-3 text-slate-300" />
        ...
        <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <RouterLink
            ...
            class="group flex flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] transition-all duration-300 hover:-translate-y-1 hover:border-emerald-200 hover:shadow-[0_10px_20px_-6px_rgba(16,185,129,0.15)]"
          >
            <!-- Image Area -->
            ...
            <div class="flex flex-1 flex-col p-5">
              <p class="mb-1 text-[11px] font-semibold uppercase tracking-widest text-slate-400 group-hover:text-emerald-600 transition-colors">
                {{ typeLabel[prop.property_type] ?? prop.property_type }}
              </p>
              
              <h3 class="line-clamp-2 min-h-[3rem] text-base font-bold leading-snug text-slate-900 group-hover:text-emerald-700">
                {{ prop.title }}
              </h3>
              
              <p v-if="prop.city" class="mt-2 flex items-center gap-1.5 text-xs text-slate-500">
              ...
              <div class="mt-auto pt-4">
                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3 text-xs font-semibold text-slate-600">
                    ...
                    <span v-if="prop.floor_area" class="flex gap-1 font-normal text-slate-500">
                      {{ prop.floor_area }} sqm
                    </span>
                  </div>
                  <p class="text-base font-extrabold text-[#0F2044]">
