...
<template>
  <div class="theme-page mx-auto max-w-3xl px-4 py-8 sm:px-6">
    ...
    >
      ...
      <h3 class="mt-3 text-sm font-bold text-rose-900">
    ...
    >
      <div class="relative mb-6">
        <div
          class="absolute -inset-4 rounded-full bg-brand-50 opacity-50 blur-xl animate-pulse"
        ></div>
        <div
          class="relative flex size-20 items-center justify-center rounded-2xl bg-brand-100 text-brand-600 shadow-inner ring-1 ring-brand-200"
        >
    ...
    <div v-else class="grid gap-5">
      ...
      >
          ...
          <div class="flex flex-col items-end gap-2">
            ...
            >
              <span
                ...
                :class="
                  statusConfig[booking.status]?.dotClass ?? 'bg-slate-400'
                "
              ></span>
        ...
        <!-- Hover indicator -->
        <div
          class="absolute inset-y-0 right-0 w-1 bg-brand-600 opacity-0 group-hover:opacity-100 transition-opacity"
        ></div>
      </RouterLink>
    </div>
  </div>
